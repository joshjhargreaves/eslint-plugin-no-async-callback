...
const isSecondArgFunctionExpression = node =>
  node.arguments && node.arguments.length > 1 && node.arguments[1].type === 'FunctionExpression';

const doesNodeHaveParams = (node: any) =>
  node.params && node.params.length > 0

module.exports = {
    meta: {
        docs: {
            description: "Async tests must use async functions and async-await instead of 'done' arguments.",
